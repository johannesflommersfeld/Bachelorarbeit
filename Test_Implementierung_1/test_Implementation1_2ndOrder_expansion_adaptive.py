...
def heun(f_start, t0, t1, dt):
	...
	global B
	while t < t1:
		f_int1 = f_final + dt*G(A,f_final)
		f_int2 = f_final + dt/2.*G(A,f_final)
		...
		if(t == 0):		
			A_int = np.linalg.inv((1-dt)*np.eye(5)/f_int1 + dt*function1.d_md_nE(f_int1))
		else:
			B_int = create_B(A_int, f_int1,t + dt)
			A_int = A_int - dt*A_int*B_int*A_int 
		B = create_B(A, f_final,t)
		f_final = f_final + dt/2*(G(A,f_final) + G(A_int,f_int1))
		A = A - dt*(A*B)*A
 		t += dt 
		return f_final
