...
class SearcherNH(object):
    ...
    #r.h.s of the ode
    def df(self,f,t):
        A = (1-t)*np.eye(self.sg.dim**2)/self.f + t*self.sg.d_md_nE(self.f)
        A_inv = np.linalg.inv(A)
        return -A_inv.dot(self.sg.dE(f))
    ...
    def heun(self):
        ...
        f_int = self.f
        f_final = self.f
        while abs(t - self.t1) > 2*self.dt:
            df_final = self.df(f_final,t)
            ...
            #corrector step
            df_int = self.df(f_int,t+self.dt)
            n=0
