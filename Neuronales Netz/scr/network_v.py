...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, dt, t1,
	    ...
	    dt = mini_batch_size/float(n)
            for mini_batch in mini_batches:
                self.update_mini_batch(mini_batch, dt)
            if test_data:
    ...
    def update_mini_batch(self, mini_batch, dt):
	...
	vs_int = [v - dt*3000*dv for v, dv in zip(self.vs, dvC)]  
	...
	self.vs = [v - dt/2.*3000*(dv+dv_int) for v,dv,dv_int in zip(self.vs,dvC, dvC_int)]	
